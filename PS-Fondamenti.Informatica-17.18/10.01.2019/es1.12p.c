/*
 * Date due matrici diagonali quadrate (chiamate 'A' e 'B') scrivere i seguenti metodi:
...
struct matrice *calculate_product(struct matrice *matrix_A, struct matrice *matrix_B){
  ...
  /* Puoi calcolare il prodotto solo se il numero delle colonne della prima matrice 
  ...
  // FIXME: Remember to free() | Create a new matrix with the compliant dimension
  struct matrice * matrix_C = populate_empty_matrix(matrix_A->n_colonne,matrix_B->n_righe,"PRODUCT");
  int i,j,k;
  ...
  for (i=0;i<M;i++) 
    for(j=0;j<P;j++) {
      ...
      for(k=0;k<N;k++)    
        matrix_C->matrix[i][j]=matrix_C->matrix[i][j]+matrix_A->matrix[i][k]*matrix_B->matrix[k][j];
    }
  return matrix_C;
}
