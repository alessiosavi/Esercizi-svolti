...
int main(){
  ...
  if (std_dev_A<std_dev_B){
    ...
    log_debug("La lista A ha std_deviation più bassa [%f] della lista B [%f]",std_dev_A,std_dev_B);
    log_debug("La media della lista A è --> %f",_std(array_A,array_lenght));
  }else{
    min_std_array = array_B;
    min_std_val = std_dev_B;
    log_debug("La lista B ha std_deviation più bassa [%f] della lista A [%f]",std_dev_B,std_dev_A);
  ...
  array_lowerized = lowerize_by_std_value(min_std_array,array_lenght,min_std_val);
