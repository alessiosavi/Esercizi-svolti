...
add_node (node_t * head, int value)
{
  ...
  if (head == NULL)
  {
    ...
    head->index = c;
    log_debug ("add_node | Adding first node_t with value %d @position %d",
        head->value, c);
    return head;
  }
  ++c;
  ...
  {
    tmp = tmp->next;
    log_debug ("add_node | Value: %d | Index: %d", tmp->value, tmp->index);
    ++c;
  }
  tmp->next = init_node (value);
  tmp->next->previous = head;
  tmp->next->index = c;
  log_debug ("add_node | Adding node_t with value %d @position %d",
      tmp->next->value, c);
  return head;
}
...
remove_node (node_t * head, int value)
{
  ...
  if (tmp->value == value){
    log_debug ("remove_node | Removing last item %d in position %d", value,counter);
    tmp->previous = tmp->previous->previous;
    tmp = tmp->next;
    return head;
  }
...
void print_node(node_t *head){
  int size = 0;
  if (head == NULL){
    log_error("print_node | No node :/");
    return ;
  }

  log_debug("print_node | Value: %d | Index: %d",head->value,size);
