...
node_t*
get_last_element (node_t *head);

int
count_node (node_t *head);
...
add_node (node_t *head, int value)
{
  ...
  if (head == NULL)
  {
    ...
    {
      //      log_debug("add_node | Iterating list ... | Value: %d | Index: %d", tmp->value, tmp->index);
      ++c;
    }
    //   log_debug("add_node | Iterating list ... | Value: %d | Index: %d", tmp->value, tmp->index);
    ...
    tmp->next = new_node;
    new_node->index = c;
    log_debug("add_node | Adding node with value %d @position %d",new_node->value, c);
  }
  return head;
}
